...
#define TYPE_THEAD_TIMER     "csky_timer_dummy"
#define THEAD_TIMER(obj)     OBJECT_CHECK(csky_timer_dummy_state, (obj), \
                                         TYPE_THEAD_TIMER)
...
static void csky_timer_dummy_tick0(void *opaque)
{
    ...
    ptimer_transaction_begin(s->timer[0]);
    csky_timer_dummy_reload(s, 1, 0);
    ptimer_transaction_commit(s->timer[0]);
    s->int_level[0] = 1;
...
{
    csky_timer_dummy_state *s = (csky_timer_dummy_state *)opaque;
    ptimer_transaction_begin(s->timer[1]);
    csky_timer_dummy_reload(s, 1, 1);
    ptimer_transaction_commit(s->timer[1]);
    s->int_level[1] = 1;
...
{
    csky_timer_dummy_state *s = (csky_timer_dummy_state *)opaque;
    ptimer_transaction_commit(s->timer[2]);
...
{
    csky_timer_dummy_state *s = (csky_timer_dummy_state *)opaque;
    ptimer_transaction_commit(s->timer[3]);
...
{
    csky_timer_dummy_state *s = THEAD_TIMER(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    s->freq[0] = 1000000000ll;
    s->timer[0] = ptimer_init(csky_timer_dummy_tick0, s,  PTIMER_POLICY_LEGACY);
    sysbus_init_irq(sbd, &s->irq[0]);

    s->freq[1] = 1000000000ll;
    s->timer[1] = ptimer_init(csky_timer_dummy_tick1, s, PTIMER_POLICY_LEGACY);
    sysbus_init_irq(sbd, &s->irq[1]);

    s->freq[2] = 1000000000ll;
    s->timer[2] = ptimer_init(csky_timer_dummy_tick2, s, PTIMER_POLICY_LEGACY);
    sysbus_init_irq(sbd, &s->irq[2]);

    s->freq[3] = 1000000000ll;
    s->timer[3] = ptimer_init(csky_timer_dummy_tick3, s, PTIMER_POLICY_LEGACY);
